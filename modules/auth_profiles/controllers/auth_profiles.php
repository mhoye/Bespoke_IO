...
{ 
    ...
    public function register()
    {
	    ...
	    //Takes care of validating new user info
	    ...
	    //Takes care of validating the admin adding the new user info
	    $admin_user = ORM::factory('profile', $form_data['admin']);
	    $admin_user_profile_id = $admin_user->__get('id');
	    $admin_role = ORM::factory('role', 'admin');
	    //$admin_role_id = $admin_role->__get('id');

	    $pks = $admin_role->load_relations('profiles_roles', $admin_user);

	    if (in_array($admin_user_profile_id, $pks)) {
	        //Check password: 
	        $login = ORM::factory('login', $form_data['admin']); 
	        $login->is_password_correct($form_data, 'admin_pwd');
	        if ($form_data->errors === array()) {
