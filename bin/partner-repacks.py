...
BUILD_NUMBER = '1'
STAGING_SERVER = 'ftp.mozilla.org'
...
#########################################################################
def parseRepackConfig(file, platforms, app):
    config = {}
    config['platforms'] = []
    f= open(file, 'r')
    for line in f:
...
#########################################################################
def getFilename(version, platform, file_ext, application):
    ...
    if application=='firefox':
        ...
        if isWin(platform):
            return "Thunderbird Setup %s.%s" % (version, file_ext)
...
class RepackBase(object):
    def __init__(self, build, partner_dir, build_dir, working_dir, final_dir,
                 platform_formatted, repack_info, version):
    ...
    def copyFiles(self, platform_dir):
        ...
        if not os.path.exists(platform_dir):
            mkdir(platform_dir)
            for i in ['distribution', 'extensions', 'searchplugins', 'greprefs', 'defaults', 'byob.cfg']:
                full_path = "%s/%s" % (self.full_partner_path, i)
                if os.path.exists(full_path):
                    if i == 'byob.cfg':
                        copy(full_path, "%s/%s" % (platform_dir, i))
                    else:
                        copytree(full_path, "%s/%s" % (platform_dir,i))
            self.createOverrideIni(platform_dir)
        if self.repack_info.get('report_off') == '1':
            self.disableReporting()
        if self.repack_info.get('defaultprefs') == 'true':
            shellCommand("%s a \"%s\" defaults/pref/byob.js" % (SEVENZIP_BIN, self.build))

    def disableReporting(self):
        if os.path.exists("core/application.ini"):
            pass
        else :
            shellCommand("%s e \"%s\" core/application.ini" % (SEVENZIP_BIN, self.build ))
            shellCommand("%s d \"%s\" core/application.ini" % (SEVENZIP_BIN, self.build ))
            os.rename("application.ini", "application.ini.bak")
            n = open("application.ini", 'w')
            b = open("application.ini.bak")
            notfound = 1
            readingfile = 1
            temp = ""
            while  readingfile == 1:
            ...
            n.close
            b.close
            os.unlink("application.ini.bak")
            move("application.ini", "core")
...
class RepackWin(RepackBase):
    ...
    def makeshorttempdir(self):
        start = 0
        while os.path.exists(TEMP_DIR + str(start)): start+=1
        working_directory = TEMP_DIR + str(start) 
        os.mkdir( working_directory )

	# I know. Believe me, I know. Right now, you're looking at this code wondering
	# what the hell this idiot was thinking, if "thinking" isn't too strong a term.
	#
	# There wasn't any other way. At least, none that was faster or safer. 
	#
	# The MSI stuff we're working with at the time this code was written have this one
	# horrible, horrible bug to them: if the length of the _absolute_ path is longer
	# 96 characters, they die (silently, because we can't put instrumentation in the
	# right places) for no reason that anyone can determine. This is why we're not using
	# mktempdir(), because they make directories with long cryptohash names that may or 
	# may not mean anything in them breaks that 96 character limit.
	#
	# And that knowledge was not won lightly, no sir.
	#
	# I know it's ugly. I know it looks dumb. I know. But unless you have a very
	# fine-grained understanding of this codebase, the environment it runs in
	# and an excellent reason for doing so, don't touch this.
...
#                      help="Set the path to the pkg-dmg for Mac packaging")
    ...
    parser.add_option("",
                      ...
                      action="store",
                      dest="application",
                      help="Specifies what application should be repackaged.") 
...
#
#  	So, this used to all point at stage.mozilla.org, which is verboten for external
#	to mozilla use. Fortunately, https:// links don't suffer the redirection that was
#	hurting us with the http redirects, so now https it is! 
#	
#
#	https://ftp.mozilla.org/pub/mozilla.org/firefox/releases/7.0.1/win32/en-US/Firefox%20Setup%207.0.1.exe
#
#	is the broad format of the url. That shouldn't change much, I hope.
# 
#	for refereence, it was: 
#
#	http://stage.mozilla.org/pub/mozilla.org/firefox/nightly/7.0.1-candidates/build1/win32/en-US/Firefox%20Setup%207.0.1.exe
#
#
    ...
    if options.application == 'firefox':
        ...
        candidates_web_dir = "/pub/mozilla.org/thunderbird/releases/%s" % ( options.version)
        win_candidates_web_dir = candidates_web_dir

# 	Commented out - never use unsigned builds.
#
...
#        win_candidates_web_dir = candidates_web_dir + '/unsigned'
    ...
    for partner_dir in os.listdir(options.partners_dir):
        ...
        if not os.path.exists(repack_cfg):
            ...
            continue
        repack_info = parseRepackConfig(repack_cfg, options.platforms, options.application)
        if not repack_info:
            continue

        partner_repack_dir = "%s/%s" % (repacked_builds_dir, partner_dir)
        print "partner_repack_dir: %s " % (partner_repack_dir)
        if not options.verify_only:
                ...
                if not options.verify_only:
                    if os.path.exists(local_filename):
                        ...
                        if options.version.startswith('3.0'):
                            original_build_url = "https://%s%s/%s" % \
                                                 (options.staging_server,
                                                  candidates_dir,
                                                  filename
                                                 )
                        else:
                            original_build_url = "https://%s%s/%s/%s/%s" % \
                                                 (options.staging_server,
                                                  candidates_dir,
                                                  platform_formatted,
                                                  locale,
