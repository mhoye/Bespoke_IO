<?php
/**
 * Thunderbird addons helper
 *
 * @package    BYOB
 * @subpackage helpers
 * @author     l.m.orchard <l.m.orchard@pobox.com>
 */
class thunderbird_addons_core {
    ...
    {
        $addon_ids = Kohana::config('thunderbird_addons.popular_extension_ids');
        $addons = array();
        foreach ($addon_ids as $id) {
            $addon = Model::factory('thunderbird_addon')->find($id);
    ...
    {
        $persona_urls = Kohana::config('thunderbird_addons.popular_personas_urls');
        $personas = array();
    ...
    {
        $addon_ids = Kohana::config('thunderbird_addons.popular_theme_ids');
        $addons = array();
        foreach ($addon_ids as $id) {
            $addon = Model::factory('thunderbird_addon')->find($id);
            if ($addon->loaded) $addons[$id] = $addon;
