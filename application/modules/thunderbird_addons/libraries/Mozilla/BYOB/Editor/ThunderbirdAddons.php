 ...
 */
class Mozilla_BYOB_Editor_ThunderbirdAddons extends Mozilla_BYOB_Editor {

    public static $max_extensions = 2;
    public static $max_search_plugins = 3;

    /** {{{ Object properties */
    public $id        = 'thunderbird_addons';
    public $title     = 'Addons';
    public $application = 'thunderbird';
    public $view_name = 
        'thunderbird_repacks/edit/edit_thunderbird_addons';
    public $review_view_name = 
        'thunderbird_repacks/edit/review_thunderbird_addons';
    public $repack = null;
    ...
    { 
        ...
        Event::add('BYOB.thunderbird_builds.perform.afterConfig',
        ...
        $self->personas_extension_id =
            Kohana::config('thunderbird_addons.personas_extension_id');
    ...
    {
        return $repack->checkPrivilege('thunderbird_addons');
    }
    ...
    public function validate(&$data, $repack, $set=true)
    {
        ...
        $unlimited_addons = 
            $repack->checkPrivilege('thunderbird_addons_unlimited');
        $max_extensions = 
        ...
        $popular_extensions = 
            thunderbird_addons::get_popular_extensions();
        $popular_personas = 
            thunderbird_addons::get_popular_personas();
        $popular_themes = 
            thunderbird_addons::get_popular_themes();
        $popular_searchplugins = 
            thunderbird_addons::get_popular_searchplugins();
        ...
        if (!empty($data['persona_url'])) {
            ...
            $persona = Model::factory('thunderbird_persona')
        ...
        if (!empty($repack->managed_addons)) {
            ...
            if (is_dir($xpi_dir)) {
                ...
                foreach ($xpi_files as $xpi_fn) {
                    $extension = Model::factory('thunderbird_addon')->find_by_xpi_file($xpi_fn);
                    if ($extension->loaded) $extensions[] = $extension;
                }
            }

            foreach ($managed_addons['extension_ids'] as $id) {
                $extension = Model::factory('thunderbird_addon')->find($id);
                if ($extension->loaded) $extensions[] = $extension;
            }
            ...
            foreach ($repack->locales as $locale) {
                ...
                if (is_dir($sp_dir)) {
                    ...
                    foreach ($sp_files as $fn) {
                        ...
                        $plugin = Model::factory('thunderbird_searchplugin')->loadFromXML($xml);
            ...
            if (!empty($managed_addons['theme_id'])) {
                $theme = Model::factory('thunderbird_addon')->find($managed_addons['theme_id']);
    ...
    public function prepareBuild()
    {
        ...
        $lightning = false;
        if (isset($repack->include_lightning)) {
            $lightning = in_array('lightning', $repack->include_lightning);
            //$fh = fopen("/home/donna/log.txt", 'a');
            //fwrite($fh, "prepareBUild: lightning - " . (string)$lightning. "\n");
            //fclose($fh);
        }
        ...
        if (!empty($repack->managed_addons)) {
            ...
            if ($lightning) {
                $extension = Model::factory('thunderbird_addon')->find(2313);
                if ($extension->loaded)
                    $extensions[] = $extension;
            }
            if (!empty($managed_addons['theme_id'])) {
                $extension = Model::factory('thunderbird_addon')
