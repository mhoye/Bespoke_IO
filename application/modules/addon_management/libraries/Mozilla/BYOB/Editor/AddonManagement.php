...
class Mozilla_BYOB_Editor_AddonManagement extends Mozilla_BYOB_Editor {
    ...
    public function validate(&$data, $repack, $set=true)
    {
        ...
        $data->add_rules('lock_addons', 'is_array');
        ...
        if (!empty($data->{'lock_addons'})) ++ $repack->{'numLockablePrefs'};
    ...
    public function prepareBuild()
    {
        ...
        if (!empty($repack->managed_addons)) {
            ...
            foreach ($extensions as $extension) {
                ...
                /* Eg.
                $extension_path =/home/donna/Desktop/feralsource/byob/trunk/addons/testpilot@labs.mozilla.com.xpi */ 
                #Part of ticket # 20 solution
                if ($repack->{'lock_addons'}) {
                    $extensionDest = "$repack_dir/extensions/";
                    if (!is_dir("$extensionDest"))
                        mkdir("$extensionDest", 0775, true);
                    $zip = new Archive_Zip("{$extension_path}");
                    $zip->extract(array('add_path' => "$extensionDest/{$extension->guid}"));
                }    
                else {
                    copy(
                        $extension_path,
        ...
        } else if (!empty($repack->certificates)) {
            self::recurseCopy($this->assets_dir, "{$repack_dir}");
        }

        /* Check that the addons and the persona are to be locked. If so, 
            use $extensions_for_install to grab the guid and the version number.
            Assign the resulting string to a Repack_Model variable */
        if ($repack->{'lock_addons'}) {
            $ext = '"';
            if (empty($extensions_for_install))
                $repack->{'enabledAddons'} = "";
            else {
                foreach ($extensions_for_install as $idx=>$extension) {
                    $email_format_guid = strstr($extension->guid, "@");
                    if ($email_format_guid != false) {
                        $ext .= $extension->guid.":".$extension->version.",";
                    } else {
                        $ext .= "{$extension->guid}:{$extension->version},";
                    }
                }
