        ...
        <div class="sub-tab-set">
            ...
            <div class="sub-tab-content selected" id="tab-extensions">
                ...
                <fieldset>
                    <ul class="lock_pref">
                        <?php 
                            $repackAddon = form::value('lock_addons');
                            if (empty($repackAddon)) $repackAddon = array();
                        ?>
                        <?php foreach (Repack_Model::$lockablePrefs as $name=>$label): ?>
                            <?php if ($name == 'addons'): ?>
                                <li>
                                <?= form::checkbox('lock_addons[]', $name, in_array($name, $repackAddon))?> 
                                <?=_('<acronym title="End users will not be able to change this preference once the browser is deployed"><b>Lock the selected addons</b></acronym>')?>
                                </li>
                            <?php endif ?>
                        <?php endforeach ?>
                    </ul>
