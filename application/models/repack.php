...
class Repack_Model extends ManagedORM
{
    ...
    public static $update_choices = array(
        'add-ons' => 'Check for updates to add-ons',
        'search'  => 'Check for updates to search plugins',
        'firefox' => 'Automically update Firefox'
    );
    ...
    public static $tab_features = array (
        'new_tab'           => 'Open new windows in a new tab instead',
        'warn_close_tabs'   => 'Warn me when closing multiple tabs',
        'warn_open_tabs'    => 'Warn me when opening multiple tabs might slow down Firefox',
        'hide_tabbar'       => 'Always hide the tab bar',
        'load_tab_bkgd'     => 'When I open a link in a new tab, switch to it immediately',
        'show_tab_preview'  => 'Show tab previews in the Windows taskbar',
        'sysaddon'          => 'Disable addon notifications',
    );

    public static $ie_imports = array(
        'ie_homepage'  => 'ie_homepage', 
        'ie_bookmarks' => 'ie_bookmarks',
        'ie_proxy'     => 'ie_proxy',
    );

    public static $numLockablePrefs = '0';
    ...
    {
        /* First check if section = 'locale_selection' || 'addon_management' is 
            a module (hence Mozilla_BYOB_EditorRegistry) */ 
        ...
        if (null !== $editor) {
            ...
            switch ($section) {
                ...
                case 'general':
                    ...
                    $data->add_rules('ffversion', 'required');
                    break;
                */
                /* case 'platforms':
                ...
                case 'bookmarks':
                    ...
                    $data->add_rules('ie_bookmarks', 'is_array');
                    break;
                /*case 'collections':
                    ...
                    break; */
                case 'homepage':
                    $data->add_rules('home_url', 'length[0,255]', 'url');
                    $data->add_rules('lock_homepage', 'is_array');
                    $data->add_rules('ie_homepage', 'is_array');
                	  break;
                case 'tabs':
                    $data->add_rules('lock_tabs', 'is_array');
                    $data->add_rules('tab_features', 'is_array');
                    $data->add_rules('webgl', 'is_array');
                    $data->add_rules('lock_webgl', 'is_array');
                    break;
                case 'proxy':
                    $data->add_rules('proxy_setting', 'required');
                    $data->add_rules('http_proxy', 'length[0,255]');
                    $data->add_rules('http_proxy_port', 'digit');
                    $data->add_rules('ssl_proxy', 'length[0,255]');
                    $data->add_rules('ssl_proxy_port', 'digit');
                    $data->add_rules('ftp_proxy', 'length[0,255]');
                    $data->add_rules('ftp_proxy_port', 'digit');
                    $data->add_rules('gopher_proxy', 'length[0,255]');
                    $data->add_rules('gopher_proxy_port', 'digit');
                    $data->add_rules('socks_host', 'length[0,255]');
                    $data->add_rules('socks_host_port', 'digit');
                    $data->add_rules('socks_version', 'required');
                    $data->add_rules('no_proxy_for_address', 'length[0,255]');
                    $data->add_rules('proxy_config_url', 'length[0,255]');
                    $data->add_rules('lock_proxy', 'is_array');
                    $data->add_rules('ie_proxy', 'is_array');
                    break;
                case 'updates':
                    $data->add_rules('updates', 'is_array');
                    $data->add_rules('lock_updates', 'is_array');
                    $data->add_rules('update_firefox', 'required');
                    $data->add_rules('notices', 'required');
                    $data->add_rules('report_off', 'is_array');
                    break;
                case 'ntlm':
                    $data->add_rules('ntlm_uris', 'length[0,255]');
                    break;
                case 'sync':
                    $data->add_rules('sync', 'required');
                    $data->add_rules('alt_server', 'depends_on[sync]');
                    $data->add_rules('lock_sync', 'is_array');
            }
        ...
        $this->numLockablePrefs = 0;
        if ($this->{'lock_homepage'}) ++$this->numLockablePrefs;
        if ($this->{'lock_proxy'}) ++$this->numLockablePrefs;
        if ($this->{'lock_updates'}) ++$this->numLockablePrefs;
        if ($this->{'lock_sync'}) ++$this->numLockablePrefs;
        if ($this->{'lock_tabs'}) ++$this->numLockablePrefs;
        if ($this->{'lock_webgl'}) ++$this->numLockablePrefs;
...
//                'en-US' : $this->default_locale;
        ...
        if ('post' != request::method()) {
            ...
            foreach (array('toolbar', 'menu') as $kind) {
                ...
                if (empty($item_in)) {
                    ...
                    if ('menu' === $kind && count($item['items']) > 8) {
                        $valid->add_error($field, 'too_many_menu');
                    } else if ('toolbar' === $kind && count($item['items']) > 8) {
    ...
    {
	if (empty($this->ffversion))
		$query = 'SELECT id FROM products ORDER BY created DESC LIMIT 1';
	else
	    $query = 'SELECT id FROM products WHERE version = \'' . $this->ffversion . '\' ORDER BY created DESC LIMIT 1'; 
	
        // Force upgrade the repack to the latest product on save.
        $this->product_id = $this->db->query($query)->current()->id; 
	...
	/** 
	 * Either customizes or inserts the the CCK extension in the repack.
	 * Called by builds.php::performBuild() 
	 *
	 * @param string Name of process
     */
	public function useCCKExtension($activity) {

        /* Determine the name of the addon */
        $addon = "bespoke-cck@extensions.feralspace.com.xpi";
        $addonUnzipped = "bespoke-cck@extensions.feralspace.com";

        if ($activity === 'customize') {
            $addonSource = Kohana::config('addon_management.dir')."/{$addon}";
            $addonDestination = Kohana::config('repacks.workspace').
                "/customizations/{$this->profile->screen_name}_{$this->short_name}";
            $copy = 'cp';
        } else if ($activity === 'insert') {
            /* 
            $addonSource = Kohana::config('repacks.workspace').
	           "/customizations/{$this->profile->screen_name}_{$this->short_name}/{$addon}";
	        $addonDestination = Kohana::config('repacks.workspace').
                "/partners/{$this->profile->screen_name}_{$this->short_name}/extensions";
            $copy = 'cp'; 
            */
            ...
            $addonSource = Kohana::config('repacks.workspace').
	           "/customizations/{$this->profile->screen_name}_{$this->short_name}/{$addon}";
	        // Destination is C:\\Program Files\Mozilla Firefox\extensions
	        // The extensions there are not in an xpi format
            $addonDestination = Kohana::config('repacks.workspace').
                "/partners/{$this->profile->screen_name}_{$this->short_name}/extensions";
            
        } 

        if (!file_exists($addonDestination))
            mkdir($addonDestination, 0775, true);
        if (is_file($addonDestination."/{$addon}"))
            unlink($addonDestination."/{$addon}");
    
        exec("{$copy} {$addonSource} {$addonDestination}/{$addon}");

        if ($activity === 'customize') {
            $this->customizeCCKExtension();
        } else if ($activity === 'insert') {
            $this->appendToConfigIni();
        }
    }
   
    /* Customize bespoke-cck@extensions.feralspace.com.xpi */

    public function customizeCCKExtension () {
    	/**	Find files, open them, load into an array, change the array, and 
    		re-write the file using the array */
        $addon = "bespoke-cck@extensions.feralspace.com.xpi";
        $addonUnzipped = "bespoke-cck@extensions.feralspace.com";
        
        $custom_dir = Kohana::config('repacks.workspace').
                "/customizations/{$this->profile->screen_name}_{$this->short_name}";
        $certificates = glob("{$custom_dir}/*.pem");
        $origdir = getcwd();
        chdir($custom_dir);

        /* Extract the addon */
        exec("7za x {$addon} -o\"{$addonUnzipped}\"");
        chdir($addonUnzipped);

        //=======================================
        /* 1) cck.config */
        /* Load addonFolder/cck.config into an array */

        $cckConfig = file("cck.config");
     
        /* Write out what needs to be inserted into cck.config */
        $certPaths = array();
        $i = 1;
        foreach ($certificates as $cert) {
            $certPaths[] = "CertPath".$i."=".$cert."\n"; //write CertPathx
            $certTrust = "CertTrust".$i."=C,C,C\n";
            $certPaths[] = $certTrust;
            $i++;
        }
        
        /* Find where to insert $certPaths[], which is where "CertPath1" is */
        $idx = 0;
        $configLength = count($cckConfig);
        for ($idx = 0; $idx < $configLength; $idx++) {
            if (0 === strpos($cckConfig[$idx], 'CertPath1'))
                break;
        }

        /* Slice the $cckConfig array into 2 parts, which cuts out the CertPath1 and CertTrust1 lines */
        $cckConfigPt1 = array_slice($cckConfig, 0, $idx);
        $cckConfigPt3 = array_slice($cckConfig, $idx + 2);
        $cckConfig = array();
        $cckConfig = array_merge($cckConfigPt1, $certPaths, $cckConfigPt3);

        /* Write $cckConfig onto addonFolder/cck.config */
        $fh = fopen("{$custom_dir}/{$addonUnzipped}/cck.config", 'w');
        foreach ($cckConfig as $cc)
            fwrite($fh, $cc);
        fclose($fh);
       
        //=======================================        
        /* 2) chrome/cck.jar */
        /* Extract chrome/cck.jar file into chrome/content */

        chdir("{$custom_dir}/{$addonUnzipped}/chrome/");
        exec("7za x cck.jar");
        
        /* Copy the pem files from the customizations folder to its appropriate location 
        	in the unzipped cck.jar file (in content/cck/). Also write out $certFiles[], 
            which will contain lines that need to be inserted to cck.properties */
      
        $cckDir = "{$custom_dir}/{$addonUnzipped}/chrome/content/cck";
        chdir($cckDir);
        $delete = glob("{$cckDir}/*.pem");
        ...
        foreach ($certificates as $c) {
            $cert_filename = basename($c);
            copy("{$c}", "{$cckDir}/{$cert_filename}");
            $certFiles[] = "Cert".$i."=".$cert_filename."\n";
            $certFiles[] = "CertTrust".$i."=C,C,C\n";
            $i++;
        }

        //=======================================
        /* 3) chrome/content/cck/cck.properties */

        //$cckProperties = file("{$custom_dir}/{$addonUnzipped}/chrome/content/cck/cck.properties");
        $cckProperties = file("cck.properties");
        $propertiesLength = count($cckProperties);
        for ($idx = 0; $idx < $propertiesLength; $idx ++) {
            if (0 === strpos($cckProperties[$idx], 'Cert1'))
                break;
        }

        $cckPropertiesPt1 = array_slice($cckProperties, 0, $idx);
        $cckProperties = array();
        $cckProperties = array_merge($cckPropertiesPt1, $certFiles);
        
        //$fh = fopen("{$custom_dir}/{$addonUnzipped}/chrome/content/cck/cck.properties", 'w');
        $fh = fopen("cck.properties", 'w');
        foreach ($cckProperties as $cc)
	        fwrite($fh, $cc);
	    fclose($fh);
	    	    
        chdir("{$custom_dir}/{$addonUnzipped}/chrome/");
	    
        /* 	Create cck.jar again. Remove pre-existing cck.jar, then create it out of
	    	/chrome/contents folder. */
	    if (is_file("cck.jar")) {
	    	unlink("cck.jar");
	    }
  
        /*  To avoid the infinite loop happening in Zip.php, hardcode the command 
            used to compress directory content */
        /*  In order for the cck.jar to be read properly by the Extension manager, first
            create a cck.zip, then rename it to cck.jar */
        exec("7z a cck.zip content/cck/*");
        rename("cck.zip", "cck.jar");

        //if (is_dir("content"))
        //    exec("rm -r content");
        //=======================================
        /* 4) Create the extension */
	    /* Work on creating the .xpi file again */
	    chdir($custom_dir);
        ...
        /*  To avoid the infinite loop happening in Zip.php, hardcode the command 
            used to compress the addon */
        /*  Create the .xpi in the directory above using the files inside $addonUnzipped,
            otherwise, file corrupt */
        $command = "7za a ../{$addon} * ";
        exec($command);
    ...
    function appendToConfigIni() {
    	$configIni = Kohana::config('repacks.workspace')."/partners/{$this->profile->screen_name}_{$this->short_name}/distribution/extensions/config.ini";
        
        $extNum = 0;

        if (!file_exists($configIni)) {
    	    $fh = fopen($configIni, 'a');
        } else {
            $configArray = array_reverse(file($configIni));
            foreach($configArray as $ca) {
                if (strpos($ca, "Extension") === 1) {
                    $extNum = (int)substr($ca, 10,1) + 1; /*Extension number for the additional addon */
                    break;
                }
            }
    	    $fh = fopen($configIni, 'a');
    	...
    	fwrite($fh,
"[Extension{$extNum}]
id=bespoke-cck@extensions.feralspace.com
file=bespoke-cck@extensions.feralspace.com.xpi
Version=1
OS=ALL");
    	fclose($fh);
    }

    /* Build the byob.cfg.txt file that will contain locked prefs */
    function buildCfg() {

        $output = View::factory('repacks/ini/byob_cfg')
            ->set('repack', $this)->render();
    ...
    /* Build the byob.js file that will point to the byob.cfg file */
    function buildPrefJS() {

        $output = view::factory('repacks/ini/byob_js')
            ->set('repack', $this)->render();
        return $output;
    } 
