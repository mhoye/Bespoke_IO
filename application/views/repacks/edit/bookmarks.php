...
<?php slot::end() ?>
<div class="intro">
    <p><?=_('You can predefine users\' bookmarks here.')?></p>
</div>
<div class="pane">
    ...
    <div>
        <ul class="lock_pref">
            <?php
                /* Bookmarks are currently not lockable */
                //$bookmarks = form::value('lock_bookmarks');
                //if (empty($bookmarks)) $bookmarks = array();
                
                $ie_bookmarks = form::value('ie_bookmarks');
                if (empty($ie_bookmarks)) $ie_bookmarks = array();
            ?>

            <?php foreach (Repack_Model::$ie_imports as $name => $label): ?>
                <?if ($name == 'ie_bookmarks'): ?>
                    <li>
                    <?= form::checkbox("ie_bookmarks[]", $name, in_array($name, $ie_bookmarks)) ?>
                    <?=_('<acronym title="End users will not be able to change this preference once the browser is deployed"><b>Import bookmarks from Internet Explorer</b></acronym>')?>
                    </li>
                    <br/>
                <?php endif ?>
            <?php endforeach ?>
    ...
    <div class="instructions">
        <h3><?=_('Notes:')?></h3>
        <ul class="notes">
            <li><?=_('If bookmarks are being imported from Internet Explorer, there is no limit to how many bookmarks could be imported.')?></li>
            <li class="toolbar-limit"><?=_('The Bookmark Toolbar has a limit of 8 items.')?></li>
            <li class="menu-limit"><?=_('The Bookmark Menu has a limit of 8 items.')?></li>
