...
<?=View::factory('thunderbird_repacks/elements/details')
    ->set('repack', $repack)->render()?> 

<?php if (!empty($changes)): ?>
<div id="changes">
    <h3><?=_('Changes')?></h3>
    <?=View::factory('thunderbird_repacks/elements/changes')
        ->set('changes', $changes)->render()?> 
</div>
<?php endif ?>

<?=View::factory('thunderbird_repacks/elements/downloads')
    ->set('repack', $repack)->render()?> 

<?php if (!empty($logevents)): ?>
<div id="history">
    <h3><?=_('History')?></h3>
    <?= View::factory('thunderbird_repacks/elements/history')->set(array(
        'logevents' => $logevents,
        'repack'    => $repack
    ))->render() ?>
</div>
