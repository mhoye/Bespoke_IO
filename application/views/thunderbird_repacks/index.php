...
<div class="intro">
    ...
    <?php if (authprofiles::is_logged_in() && 
        ...
        <button id="firefox" name="firefox" class="submit required button large yellow"><?=_('Customize Firefox')?></button>
        <button id="thunderbird" name="thunderbird" class="submit required button large yellow"><?=_('Customize Thunderbird')?></button>
        <span class="large blue button"><a href="<?=url::site("profiles/{$profile->screen_name}/browsers")?>"><?=_('View Firefox customizations')?></a></span>
        </form>
    <?php endif ?>
</div>
...
<div class="white_box">
    ...
    <div class="header">
        <?php if (authprofiles::get_profile('screen_name') == $profile->screen_name): ?>
            <h3><?=_('Your mail clients')?></h3>
        <?php else: ?>
            <?php /* i18n: %1$s = profile screen name */ ?>
            <h3><?=sprintf(_('Mail clients by %1$s'), $profile->screen_name)?></h3>
        <?php endif ?>
    </div>
            ...
            <li class="browser">
                ...
                <ul class="branches">
                    ...
                    <?php foreach (array('released', 'unreleased') as $key): ?>
                        ...
                        ?>
                        <li class="branch <?=$key?>">
                            <?=View::factory('thunderbird_repacks/elements/status')
                            ...
                            <?=View::factory('thunderbird_repacks/elements/actions')
                                ->set('repack', $repack)->render() ?>
                        </li>
